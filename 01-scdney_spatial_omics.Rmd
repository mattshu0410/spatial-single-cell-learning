...
If you are viewing this Rmd before the workshop, you might also like to download 
the data that we will use. This can take time.
...
1.  How many cells are in this data? 11026
2.  How many genes? 351
...
length(colData(spe)$cell_id)
imgData(spe)
dim(spe)
class(assay(spe, 'molecules'))
plotReducedDim(spe, )
```
...
Normalising based on the fact that smooth muscle actin (SMA) is very high frequency across all pancreatic cells.
...
spe$gutRegion <- spe$celltype_mapped_refined == "Gut tube" &
  # There is some gut in the image that we don't want so we just cut that out
...
## A very spicy analysis of the IMC data
...
### Identifying cell types in the data.
...
```{r estimateClusterNumber, fig.height= 5, fig.width=5}
...
```{r plotImage, , fig.height= 5, fig.width=6}
...
### spicyR: testing for changes in spatial relationships between cell types
...
```{r signifPlot, fig.height= 4, fig.width=6}
...
```{r violin, fig.height= 5, fig.width=5}
...
2. Are your conclusions different if you use Damond's cell type annotations?
...
### lisaClust: Describing tissue microenvrionments and cellular neighbourhoods.
...
```{r hatchingPlot, fig.height= 5, fig.width=6}
